...
MIN_VOLUME = 500000  # 거래대금 최소 기준 (원)
...
def telegram_bot_commands():
    ...
    def status_command(update, context):
        if not open_positions:
            update.message.reply_text("현재 보유 중인 포지션이 없습니다.")
        else:
            msg = ""
            for t, d in open_positions.items():
                cp = pyupbit.get_current_price(t)
                msg += f"{t}: {d['entry_price']} → {cp} (TP:{d['tp']}% / SL:{d['sl']}%)\n"
        ...
        for ticker, data in list(open_positions.items()):
            ...
            if price >= entry_price*(1+tp/100):
                ...
                if (new_tp, new_sl) != (tp, sl):
                    ...
                    send_telegram_message(f"[조건변경] {ticker}: TP:{tp}->{new_tp}% / SL:{sl}->{new_sl}%")
                open_positions[ticker]['last_checked'] = now
    except Exception as e:
        send_telegram_message(f"[루프 예외 발생] {e}")
        time.sleep(3)
    time.sleep(1)
